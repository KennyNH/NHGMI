...
def load_acm(ratio, type_num):
    ...
    return [nei_a, nei_s], [feat_p, feat_a, feat_s], [pap, psp], pos, label, train, val, test, eval_train, eval_val, eval_test
...
def load_dblp(ratio, type_num):
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_p], [feat_a, feat_p], [apa, apcpa, aptpa], pos, label, train, val, test, eval_train, eval_val, eval_test
...
def load_aminer(ratio, type_num):
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_a, nei_r], [feat_p, feat_a, feat_r], [pap, prp], pos, label, train, val, test, eval_train, eval_val, eval_test
...
def load_freebase(ratio, type_num):
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_d, nei_a, nei_w], [feat_m, feat_d, feat_a, feat_w], [mdm, mam, mwm], pos, label, train, val, test, eval_train, eval_val, eval_test
...
def load_imdb(ratio, type_num):
    ...
    nei_a = np.load(path + "nei_a.npy", allow_pickle=True)
    nei_d = np.load(path + "nei_d.npy", allow_pickle=True)
    nei_k = np.load(path + "nei_k.npy", allow_pickle=True)
    ...
    nei_a = [th.LongTensor(i) for i in nei_a]
    nei_d = [th.LongTensor(i) for i in nei_d]
    nei_k = [th.LongTensor(i) for i in nei_k]
    feat_m = th.FloatTensor(preprocess_features(feat_m))
    feat_a = th.FloatTensor(preprocess_features(feat_a))
    feat_d = th.FloatTensor(preprocess_features(feat_d))
    ...
    mam = sparse_mx_to_torch_sparse_tensor(normalize_adj(mam))
    mdm = sparse_mx_to_torch_sparse_tensor(normalize_adj(mdm))
    mkm = sparse_mx_to_torch_sparse_tensor(normalize_adj(mkm))
    pos = sparse_mx_to_torch_sparse_tensor(pos)
    train = [th.LongTensor(i) for i in train]
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_a, nei_d, nei_k], [feat_m, feat_a, feat_d, feat_k], [mam, mdm, mkm], pos, label, train, val, test, eval_train, eval_val, eval_test
